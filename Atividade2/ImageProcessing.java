...
public class ImageProcessing {
    public static void main(String[] args) {
        ...
        BufferedImage[] images = { originalImage, convertToGrayScale(originalImage), invertImageColors(originalImage),
                blurImage(originalImage, 1)
        };
    ...
    public static BufferedImage convertToGrayScale(BufferedImage originalImage) {
        ...
        for (int x = 0; x < columns; ++x) {
            for (int y = 0; y < rows; ++y) {
                ...
                pixelValue = (average << 16) + (average << 8) +
            ...
            for (int y = level; y < rows - level; ++y) {
                ...
                for (int xNeighbor = x - level; xNeighbor <= x + level; ++xNeighbor) {
                    ...
                    for (int yNeighbor = y - level; yNeighbor <= y + level; ++yNeighbor) {
                        ...
                        int pixelValue = originalImage.getRGB(
                ...
                int pixelValue = (red / neighborhoodArea << 16)
                        + (green / neighborhoodArea << 8)
                        + blue / neighborhoodArea;
